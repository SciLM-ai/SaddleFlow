...
**Cascade and Sella-retargeting are SUBSTITUTES, not complements (settled 2026-08-25).** The last untested combination was run as `SELCASC`: stage 1 = SEL1, targets regenerated by running Sella from SEL1's *own* 31,268 train-split predictions (25,018 kept, 89.9% index-1), then a refiner with `--init-weights SEL1 --start-override <SEL1 preds> --saddle-override sella_targets_v2.npz`. On the same pinned 300 cases (paired n=252) it reads NEAREST maxd median **0.1070 vs CASC_refiner's 0.1168**, 12 vs 13 Sella steps — nominally +8.3%, but **paired Wilcoxon p = 0.38, winning only 54% of cases.** It also ties against the Sella saddle (p=0.76) and the dataset label (p=0.56). Both levers repair the *same* deficiency, so stacking buys nothing. **`CASC_refiner` remains the recommendation** — one relabeling instead of two, no second Sella pass. Corollary: the earlier "+10% for SEL1 over R15" was never significance-tested and should be read as suggestive only.
