...
def parse_args():
    ...
    p.add_argument("--random-sample", action="store_true",
                   help="With --num-cases N > 0, draw N triplets uniformly at "
                        "random (seeded by --seed) instead of taking the first "
                        "N. Use for a representative subset of a large test "
                        "split (the parquet is sorted by ms_id, so first-N is "
                        "a biased contiguous slice). Default off = first-N.")
...
def main():
    ...
    if args.num_cases > 0:
        if args.random_sample:
            # Uniform random subset (seeded). Sort afterwards so the across-rank
            # sharding (`my_indices = range(n)[rank::nprocs]`) is deterministic.
            samp_rng = random.Random(args.seed * 7919 + 1)
            n_take = min(args.num_cases, len(test_tids))
            tids = sorted(samp_rng.sample(list(test_tids), n_take))
        else:
